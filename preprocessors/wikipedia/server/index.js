...
const parseMD = (...args) =>
  import("parse-md").then(({ default: parseMD }) => parseMD(...args));
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/wikitext")
      return await req.json().then(
        async (a) =>
          await wikitext_to_md(a["content"]).then(
            (res) => new Response(res, { status: 200 }),
            (err) => new Response("Malformed json payload", { status: 400 }),
          ),
        (err) => {
          console.log(err);
